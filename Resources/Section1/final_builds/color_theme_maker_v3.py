#Color Theme Maker
#Icon GNU General Public License
import tkinter
from tkinter import BOTH, DISABLED, IntVar, filedialog
...
def get_red(slider_value):
    ...
    The slider value is passed automatically when the slider is moved calling the get_red function"""
    global red_value
...
def get_green(slider_value):
    """Turn current slider value for red into a hex value and update color
    The slider value is passed automatically when the slider is moved calling the get_green function"""
    global green_value
...
def update_color():
    """Update the current color box based on the slider values.  Display tuple and hex value of current color"""
    #Make the color box smaller than the original due to ipadx and ipady on original color box.
    color_box = tkinter.Label(input_frame, text="", bg="#" + red_value + green_value + blue_value, height=6, width=15)
...
def set_color(r,g,b):
    """Set a given color, upon setting sliders get_red, get_green, get_blue functions will be called, and upadate_color will be called"""
    red_slider.set(r)
    green_slider.set(g)
    blue_slider.set(b)
...
def store_color():
    ...
    #Keep a reference of the current rgb colors.
    stored_red = red_slider.get()
    stored_green = green_slider.get()
    stored_blue= blue_slider.get()

    #Create all the information for the new stored color.  Make sure the button command is for the given color.
    ...
    new_color_box_black = tkinter.Label(output_frame, bg="#000000", width=3, height=1)
    new_color_box = tkinter.Label(output_frame, bg="#" + red_value + green_value + blue_value, width=3, height=1)

    #Put all the new objects onto the window.
    ...
    new_color_box_black.grid(row=stored_color.get(), column=4, pady=2, ipadx=5, ipady=5)
    new_color_box.grid(row=stored_color.get(), column=4)

    #Update the dict stored_colors with the new colors tuple and hex values.
    stored_colors[stored_color.get()] = [new_color_tuple.cget("text"), new_color_hex.cget("text")]

    #Move the radio button stored_colors to the next value if available.
    if stored_color.get() < 5:
        stored_color.set(stored_color.get() + 1)


def save_colors():
    """Output the chosen colors tuple and hex values to a text document"""
    #Get the directory where the user would like to save and the filename.
...
#Define GUI Layout...For LabelFrame, ipadx ipady may not work...so you can call padx pady in creation.
...
#Put labels, sliders, and buttons onto the interface frame
#Use ipadx with rgb to define column width, then sticky to fill.
...
color_box = tkinter.Label(input_frame, bg="#000000", height=6, width=15)
color_tuple = tkinter.Label(input_frame, text="(0), (0), (0)")
color_hex = tkinter.Label(input_frame, text="#000000")

#Put the color box and color labels onto the interface frame
...
for i in range(6):
    ...
    new_color_box_black = tkinter.Label(output_frame, bg="#000000", width=3, height=1)
    new_color_box = tkinter.Label(output_frame, bg="#ffffff", width=3, height=1)
    ...
    new_color_box_black.grid(row=i, column=4, pady=2, ipadx=5, ipady=5)
